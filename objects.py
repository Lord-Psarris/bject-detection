...
# getting the names of the objects
with open('yolov3/coco.names', 'rt') as f:
    classes = f.read().rstrip('\n').split('\n')

# for the colors of the boxes
...
def where_is_it(frame_, outputs_):
    frame_h = frame_.shape[0]
    frame_w = frame_.shape[1]
    bboxes, probs, class_ids = [], [], []
    for preds in outputs_:  # different detection scales
